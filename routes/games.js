...
router.get("/fromsearch", async (req, res, next) => {
  ...
  // utiliser la methode lean() qui permets d'avoir un resultats plus rapide
...
router.post("/newgames", async (req, res, next) => {
  // regarder si le jeux n'est pas déja dans la BDD
  Game.findOne({ name: req.body.name }).then((data) => {
    // si le jeu n'est pas dans la bdd, le créer
    if (!data) {
...
router.post("/addToList", async (req, res) => {
  ...
  if (!user) {
    res
      .status(400)
      .json({ result: false, error: "Username is not in the database" });
    return;
  }
  ...
  if (!list) {
    res
      .status(400)
      .json({ result: false, error: "Your list is not in the database" });
    return;
  }
  ...
  if (!game) {
    res
      .status(400)
      .json({ result: false, error: "Your game is not in the database" });
    return;
  }
  ...
  if (list.gameList.includes(gameId)) {
    res
      .status(304)
      .json({ result: false, error: "Your game is already in the list" });
    return;
  }
  ...
  res
    ...
    .json({
      result: true,
      message: "Your game has been succesfuly added to your list",
    });
});
...
router.delete("/:listName/:gameName/:username", async (req, res) => {
  try {
    ...
    if (!listName) {
      res
        .status(400)
        .json({ result: false, error: "There is no list with that name." });
      return;
    }
    ...
    if (!gameName) {
      res
        .status(400)
        .json({ result: false, error: "There is no game with that name." });
      return;
    }
    ...
    if (listName2 === "All my games") {
      ...
      if (check) {
        ...
        await List.updateOne(
          { listName: listName2, user: user._id },
          { $pull: { gameList: game._id } }
        );
