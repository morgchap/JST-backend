...
router.post("/addList", (req, res) => {
  try {
    ...
    if (!listName || listName === "") {
      res
        .status(400)
        .json({ result: false, error: "Missing name for the list" });
      return;
    }
    ...
    User.findOne({ username }).then((data) => {
      ...
      if (!userId) {
        res
          ...
          .json({
            result: false,
            error: "Missing user id, are you connected ?",
          });
        return;
      }
      ...
      List.find({ user: userId, listName }).then(async (data) => {
        if (data.length !== 0) {
          ...
          res
            .status(400)
            .json({ result: false, error: "List already in the database" });
          return;
        } else {
    ...
    User.findOne({ username }).then((data) => {
      ...
      if (!data) {
        res
          ...
          .json({
            result: false,
            error: "Didn't find the user in the database",
          });
        return;
      }
      ...
      List.find({ user: data._id }).then((data) => {
        if (data.length === 0) {
          ...
          res
            ...
            .json({
              result: false,
              error: "You don't have any list in the database",
            });
          return;
        } else {
...
router.post("/allgames", async (req, res) => {
  try {
    ...
    if (!userinfo) {
      return res
        .status(400)
        .json({ result: false, error: "Didn't find the user in the database" });
    }
    ...
    if (!list) {
      return res
        .status(400)
        .json({ result: false, error: "User's game list not found" });
    }
...
router.delete("/:listName/:username", async (req, res) => {
  try {
    ...
    if (!username) {
      res
        .status(400)
        .json({ result: false, error: "There is no username with that name." });
      return;
    }
...
router.get("/getGames/:listName/:username", async (req, res) => {
  ...
  if (!list) {
    res
      ...
      .json({
        result: false,
        error: "There is no list with that name for this username.",
      });
    return;
  }
